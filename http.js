...
let flightInfo = window.geofsFlightInfo;
  ...
  async function postJSON(path, payload, options = {}) {
    ...
    if (options.beacon && navigator.sendBeacon) {
      return navigator.sendBeacon(reportUrl(path), new Blob([body], { type: 'application/json' }));
    }
  ...
  function reportDisconnect() {
    ...
    try {
      postJSON('/api/report/disconnect', { aircraftId: lastAircraftId }, { beacon: true, keepalive: true });
    } catch (e) {}
  }
